...
comdat <- function(dr=goext,zv=c('alpha','NULL'),xv=c('lamda','Psi_s'),yv=c('lamda','Psi_t')){
  P <- dr['P']
  le <- dim(P)[1]
  dr <- dplyr::arrange(dr,P)
  ### Data
  if (is.null(zv[2])) zv[[2]]<- rep(0,le)
  z0 <- complex(real=dr[[zv[1]]],imaginary=dr[[zv[2]]])
...
#' @export abc
#' @export olsce
olsce2 <- function(dr=goext,ce=NULL,zv=c('alpha','NULL'),xv=c('lamda','Psi_s'),yv=c('lamda','Psi_t')){
  P <- dr['P']
  le <- dim(P)[1]
  dr <- dplyr::arrange(dr,P)
  ### Data
