abc <- function(fld="/data-raw/arizona/2024/",rac=c("Presidential"),aff=c("DEM","REP"),vom=c('Votes_PROVISIONAL','Votes_ELECTION DAY','Votes_EARLY VOTE')){
  lsf <- system(paste0('ls ',rprojroot::find_rstudio_root_file(),"/data-raw/arizona/2024"), intern=T)
  sapply(1:length(lsf),function(per){
    sapply(1:length(lsf),function(ran){
	snax <- iris
	setNames(list(snax),rac)
    })
  })    
}
abc()
