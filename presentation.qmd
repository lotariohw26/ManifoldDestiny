...
# abc <- seloutput(selreport(appsn))
# abc <- seloutput(selreport(apprn))
```
...
<br>
\begin{equation*}
f(x)_{0}^{1}=`r #paste0(round(penf,3))`$
\end{equation*}


Election outome 
  - Integral `r #round(polynom::integral(penf,c(0,1)),3)`
  - Counting `r #round(rppnf[[1]]$desms[7,2],3)`
...
\begin{equation*}
f(x)_{0}^{1}=`r #paste0(round(per1,3))`$
\end{equation*}
...
Election outome
```{r, echo=F}
#round(polynom::integral(per1,c(0,1)),3)
```
