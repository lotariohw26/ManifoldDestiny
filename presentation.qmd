...
```{r}
kable(laws_df1, format = "markdown", escape = FALSE) %>% kableExtra::kable_paper(full_width = F) %>% kableExtra::kable_styling(font_size=12)
```
...
\begin{equation*}
f(x)_{0}^{1}=
`r #paste0(round(polynom::polynomial(unname(pri_int_ex1)),3))`$
\end{equation*}
